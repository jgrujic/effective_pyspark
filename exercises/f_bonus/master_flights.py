...
if __name__ == "__main__":
    ...
    clean_zone = repo_root / "data" / "clean_zone"
    flights, airports, carriers = [
        spark.read.parquet(str(clean_zone / f))
        for f in ("flights", "airports", "carriers")
    ]

    business_zone =  repo_root / "data" / "business_zone"
    business_zone.mkdir(parents=True, exist_ok=True)

    master_table = produce360view(flights, airports, carriers)

    master_table.write.parquet(str(business_zone / "flights360"), mode="overwrite")
